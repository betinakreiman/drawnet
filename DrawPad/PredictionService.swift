...
struct PredictionService {
    static func predict(img: UIImage, module: TorchModule) -> [NSNumber] {
        
        //var betinaImage = UIImage(named: "tthree")!
        //var betinaImage = CGImage(UIImage(named: "tthree"))!
        ...
        /* var raw = [UInt8]()
        for i in 0...783 {
          raw.append(rawBytes[i])
        }
        print(raw) */
      
        
        //let formattedPixels: [UInt8] = reorientImage(from: rawBytes)
        //var normalizedPixels: [Float32] = normalizeData(data: rawBytes)
        var normalizedPixels: [Float32] = betinaNormalize(data: rawBytes)
        
        /* var pix = [Float32]()
        print()
        for i in 0...783 {
            pix.append(normalizedPixels[i])
        }
        print(pix)
        print() */
        
        //let rawBytes: [UInt8] = getRawBytes(from: betinaImage as! CGImage)
        //let formattedPixels: [UInt8] = reorientImage(from: rawBytes)
    ...
    private static func runInference(normalizedPixels: inout [Float32], module: TorchModule) -> [NSNumber] {
    ...
    private static func betinaNormalize(data: [UInt8]) -> [Float32] {
        var normalizedBuffer: [Float32] = [Float32](repeating: 0, count: 28 * 28)
        
        for i in 0 ..< 28 * 28 {
            let pixVal = data[i]
            var normalizedVal = 255 - (Float32(pixVal))
            normalizedVal = normalizedVal * 0.99/255 + 0.01
            normalizedBuffer[i] = normalizedVal
        }
        return normalizedBuffer
    }
